...
PHP 5.4, but has been tested for PHP 7 as well. It is available on `Github`_, through
...
maintained in the `Gitter chat room`_, the `IRC channel`_ and `Twitter`_. The `API
documentation`_ is located on the website.
...
.. _API documentation: http://api.popphp.org/2/
